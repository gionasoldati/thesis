...
\begin{document}
    ...
    \begin{frame}{Hash functions ($\simeq$ Random functions)}
    		\begin{center}
    			...
    			%\path[->,thick]
    			%([yshift=-0.3cm] a.north east) edge ([yshift=-0.3cm] d.north west)
    			%([yshift=0.3cm] d.south west) edge ([yshift=0.3cm] a.south east)
    			%([yshift=-1.1cm] b.north east) edge ([yshift=-0.3cm] e.north west)
    			%([yshift=0.1cm] e.south west) edge ([yshift=0.85cm] b.south east)
    			%([yshift=-0.3cm] c.north east) edge ([yshift=-0.3cm] f.north west)
    			%([yshift=0.3cm] f.south west) edge ([yshift=0.3cm] c.south east);
			...
			\begin{center}
				...
				\begin{minipage}{1\textwidth}
					\begin{wrapfigure}{r}{0.5\textwidth}
						\vspace*{-0.8cm}
						\hspace*{-0.72cm}   
						\includegraphics[height=5cm, width=7.5cm]{images/ec_over_ff.eps}
					\end{wrapfigure}
    ...
    \begin{frame}{Digital signature}
        \begin{center}
        	...
        	\node (c) [xshift=2.1 cm, yshift=-1cm] {\includegraphics[scale=0.1]{images/sig.jpg}};
        	\node (f) [xshift = 2.1 cm, yshift = 1cm] {\includegraphics[scale=0.06]{images/doc.png}};
			...
			\begin{column}{0.62\linewidth}
				\begin{center}
					\begin{tikzpicture}[
						...
						\only<10>{\node (i) [right=of a, xshift=-1.6cm, yshift=1cm] {\small $t_2 \stackrel{?}{=} \text{hash}(K_2)$}};
						\only<10>{\node (i) [right=of a, xshift=-1.6cm, yshift=0.5cm] {\small $t_3 \stackrel{?}{=} \text{hash}(K_3)$}};
		...
		\begin{columns}
			\begin{column}{0.51\linewidth}
				\begin{textblock*}{6cm}(1.3cm,1.1cm) 
					\only<1->{\begin{tikzpicture}[
						...
						\end{tikzpicture}}
				\end{textblock*}
				\begin{textblock*}{6cm}(0.3cm,2.4cm) 
					\only<4-9>{
						The dealer:
						\only<5-6>{
							\begin{itemize}
					...
					\only<11-12>{
						\begin{itemize}
							...
							$f(u) = f(i)\omega_i(u)$, where $\omega_i(u) = \prod_{j \neq i} \frac{u - j}{i - j} \ (\text{mod} \ n)$.
							\\
							$s = f(0) = s_i\omega_i$, with $\omega_i = \omega_i(0) = \prod_{j \neq i} \frac{j}{j - i} \ (\text{mod} \ n)$.
						\end{itemize}
					}
					}
				\end{textblock*}
				\begin{textblock*}{6cm}(0.3cm,2.8cm) 
					...
					\begin{itemize}
						...
						\item<16-17> at the end of the procedure the shared secret is $r = \sum_{i = 1}^{m}r_i$ with shares $s_i = \sum_{j = 1}^{m} f_j(i) \ (\text{mod} \ n)$;
			...
			\begin{column}{0.65\linewidth}
				\begin{tikzpicture}[
					...
					\only<11> {\node [above=of d, xshift=1.75cm, yshift=-1.6cm] {\includegraphics[scale=0.07]{images/pensiero.jpg}};}
					\only<12>{\draw [->] (d) -- (g);
						\draw [->] (g) -- (d);
				...
				\draw [->] (g) -- (a);}
				\only<21>{
					\draw [->] ([xshift=0.5cm] a) -- (g);
					\draw [->] ([xshift=-0.5cm] g) -- ([xshift=0.5cm]a);
					\node [right=of a, xshift=-2.2cm, yshift=-2cm] {$\gamma_1$};
					\node [right=of g, xshift=-3cm, yshift=2cm] {$\gamma_3$};}
				\only<22>{
