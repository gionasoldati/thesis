...
In Chapter \ref{chpr:math} we will start with the mathematical foundations of the cryptography underpinning the Bitcoin protocol: in particular we will recall the concepts of groups and fields (Section \ref{groupsfields}), we will introduce the general idea of elliptic curve (EC in Section \ref{ec}) and show how to induce the group structure on an EC through a proper definition of the addition operation (Section \ref{grouplaw}).

\bigskip
\noindent
In Chapter \ref{chpr:ecc} we will give an overview of the public key cryptography based on elliptic curves: we will start with the basic mathematical structure, the elliptic curve over a finite field (Section \ref{ecoverff}), we will discuss a standard for elliptic curve cryptography (Section \ref{ecparam}) and we will present the basic idea behind asymmetric cryptography based on EC (Section \ref{keypairs}). The chapter will be concluded by the description of the hard problem at the basis of elliptic curves' widespread adoption in recent years, the so called discrete logarithm problem (DLP in Section \ref{dlp}).

\bigskip
\noindent
Chapter \ref{chpr:dss} will deal with ECDSA (Section \ref{ecdsa}) and Schnorr (Section \ref{schnorr}). Throughout the chapter many topics will be touched, among which ECDSA issues, Schnorr solutions and multi-signature schemes.
...
\bigskip
\noindent
In Chapter \ref{chpr:conclusion} we will summarize the thesis and draw the conclusions.
